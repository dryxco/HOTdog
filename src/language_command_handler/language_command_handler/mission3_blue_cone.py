#!/usr/bin/env python3
"""
Mission 3: Move to the blue cone and bark (Corrected Frames)
...
COLOR_KEYWORDS: Dict[str, List[str]] = {
    "blue": ["blue", "blue_cone", "blue cone", "cone_blue", "blue-cone"],
}

class Mission3BlueCone(Node):
    def __init__(self):
        super().__init__("mission3_blue_cone")
    ...
    def _label_has_blue_cone(self) -> bool:
        txt = self.detected_label or ""
        return any(k in txt for k in COLOR_KEYWORDS["blue"])
    ...
    def _loop(self):
        ...
        if self.state == "OBSERVE":
            ...
            if self._label_has_blue_cone():
                self.get_logger().info("Blue cone detected. Barking.")
            ...
            if (time.time() - self.state_enter_time) > OBSERVE_SECONDS:
                ...
                if self.site_cursor >= len(SCAN_ORDER):
                    self.get_logger().warn("Blue cone not found in any site. Finishing mission.")
